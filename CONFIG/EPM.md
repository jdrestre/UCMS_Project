...
## 3. DICCIONARIO DETALLADO DE COLUMNAS
...
5. **Al_Consumo** `Consumo mmm-yy` Calculado: m3 x Costo ($) Alcantarillado.
6. **Al_Cargo_Fijo**: `Cargo fijo mmm-yy` Valor cargo fijo mensual de Alcantarillado.
7. **Al_Cont_Cargo_Fijo**: `Contr cargo fijo mi %` Valor de la contribución o subsidio sobre el cargo fijo de Alcantarillado.
8. **Al_Cont_Consumo**: `Contrib consumo min %` Valor de la contribución o subsidio sobre el consumo variable de Alcantarillado.
...
*No aplica desglose técnico adicional para Alumbrado Público.*
...
### REGLA DE INTEGRIDAD HORIZONTAL (EXTRACCIÓN TOTAL): 
Es estrictamente obligatorio que la tabla de salida contenga la totalidad de las columnas definidas en el DICCIONARIO DETALLADO DE COLUMNAS (Secciones I, II y III y IV). No se permite el truncamiento de la tabla por razones de espacio o legibilidad. Si un dato técnico no aplica al servicio de la fila o no está presente en el PDF, se debe registrar obligatoriamente como 0,00. La tabla DEBE iniciar en ID_Unico y finalizar en Info_Reg_Json sin excepciones, siguiendo el estándar de encabezados.

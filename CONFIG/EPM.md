...
20. **Ac_Consumo** `Consumo mmm-yy` Calculado: m3 x Costo ($) Acueducto.
21. **Ac_Cargo_Fijo**: `Cargo fijo mmm-yy` Valor cargo fijo mensual de Acueducto.
22. **Ac_Contr_Cargo_Fijo**: `Contr cargo fijo mi %` Valor de la contribución o subsidio sobre el cargo fijo de Acueducto.
23. **Ac_Cont_Consumo**: `Contrib consumo min %` Valor de la contribución o subsidio sobre el consumo variable de Acueducto.

### Valores facturados Alcantarillado
24. **Al_Consumo** `Consumo mmm-yy` Calculado: m3 x Costo ($) Alcantarillado.
25. **Al_Cargo_Fijo**: `Cargo fijo mmm-yy` Valor cargo fijo mensual de Alcantarillado.
26. **Al_Cont_Cargo_Fijo**: `Contr cargo fijo mi %` Valor de la contribución o subsidio sobre el cargo fijo de Alcantarillado.
27. **Al_Cont_Consumo**: `Contrib consumo min %` Valor de la contribución o subsidio sobre el consumo variable de Alcantarillado.

### Valores facturados Energía
28. **E_Energía**: `Energía mmm-yy` Valor consumo Energía kWh liquidado en el periodo.
29. **E_Contr_Activa**: `Contribución activa %` Valor de la contribución o subsidio sobre el consumo de Energía.
### Valores facturados Gas Natural
30. **G_Consumo**: `Consumo mmm-yy` Valor consumo Gas m3 liquidado en el periodo.
31. **G_Cargo_Fijo**: `Cargo fijo mmm-yy` Valor cargo fijo mensual de Gas Natural.
32. **G_Cont_Consumo**: `Contrib consumo %` Valor de la contribución o subsidio sobre el cargo fijo de Gas Natural.
33. **G_Cont_Cargo_Fijo**: `Contrib cargo fijo %` Valor de la contribución o subsidio sobre el cargo fijo de Gas Natural.

### Valores facturados Aseo
34. **As_Cargo_Fijo**: `Cargo Fijo` Valor cargo fijo mensual de Aseo.
35. **As_Variable_Apro**: `Cargo Variable Aprovechable` Valor consumo de toneladas aprovechables de Aseo.
36. **As_Contrib**: `Contribucion 100%` Contribución sobre toneladas de Aseo.
37. **As_Cargo_Var**: `Cargo Variable` Valor consumo total de Aseo.

### Valores facturados Alumbrado Público
38. **Al_Alum_Pub**: `Alumbrado Público` Valor total de Alumbrado Público.

### Columna Ajuste del peso
39. **Ajuste_Peso**: Columna técnica para ajuste interno del peso de la factura (si aplica). Relleno técnico (`0,00`) si no aplica. Se debe incluir en la Fila TOTAL.

### Columna de Totalización Final por Fila según OJF
40. **Total_Fila**: Sumatoria neta liquidada en la fila actual. (`Total Acueducto`, `Total Alcantarillado`, `Total Energía`, `Total Gas`, `Total Aseo`, `Total Alumbrado`, `Total Otras Entidades`, `Total a pagar Factura`).
...
41. **Costo**:  `Costo ($)` Costo unitario del servicio principal (Acueducto, Alcantarilla, Energía, Gas, Aseo y Alumbrado no tienen este Costo).
42. **Consumo_Cant**: Cantidad física medida (Acueducto y Alcantarillado `m3`, Energía `kWh`, Gas `m3`. Para Aseo y Alumbrando no aplica, no hay un valor definido para este item).
43. **Unidad_Medida**: Unidad de medida (m3, kWh, Ton).
44. **Medidor**: `'Serial del medidor (con apóstrofe)`. Solo aplica para Acueducto, Energía y Gas.
45. **Lect_Ant**: `Lectura anterior` Lectura anterior registrada. Solo aplica para Acueducto, Energía y Gas.
46. **Lect_Act**: `Lectura actual` Lectura actual capturada. Solo aplica para Acueducto, Energía y Gas.
47. **Const**: Factor Multiplicador (Energía) o Constante de Corrección (Gas).

#### Componentes del Costo Acueducto
48.  **Ac_Cmapac_Cost**: `Cmapac - Cost` (valor unitario de administración Acueducto).
49.  **Ac_Cmpac_Unitari**: `Cmpac Unitari`. (valor unitario Cmpac).
50. **Ac_Cmpac_Total**: `Cmpac Total`. (valor total Cmpac).
51. **Ac_Cmt Unitario**: `Cmt Unitario`. (valor unitario Cmt).
52. **Ac_Cmt_Total**: `Cmt Total`. (valor total Cmt).

### Componentes del Costo Alcantarillado
53.  **Al_Cmt_Unitario**: `Cmt Unitario` (valor unitario Cmt Alcantarillado).
54.  **Al_Cmt_Total**: `Cmt Total` (valor total Cmt Alcantarillado).

### Componentes del Costo Energía
55. **E_CU_G**: `Generación` Componente Generación Energía ($/kWh).
56. **E_CU_T**: `Transmisión` Componente Transmisión Energía ($/kWh).
57. **E_CU_D**: `Distribución` Componente Distribución Energía ($/kWh).
58. **E_CU_C**: `Comercializac` Componente Comercialización Energía ($/kWh).
59. **E_CU_P**: `Pérdidas` Componente Pérdidas Energía ($/kWh).
60. **E_CU_R**: `Restricciones`Componente Restricciones Energía ($/kWh).

### Componentes del Costo Gas Natural
61. **G_CU_Compr_V**: `Compra` Compra Gas Variable unitaria.
62. **G_CU_Dist_V**: `Distribución` Distribución Gas Variable unitaria.
63. **G_CU_Transp7_V**: `Transporte 7` Valor unitario ID Dinámico (Gas).
64. **G_CU_Conf_V**: `Confiabilidad` Confiabilidad Gas Variable unitaria.
65. **G_CU_Com_V**: `Comercializac` Comercialización Gas Variable unitaria.
66. **G_CU_Transp_Gn_F**: `Transporte Gn` Valor fijo de transporte (Gas).
67. **G_CU_Compr8_F**: `Compresión 8` Valor fijo compresión (Gas).
68. **G_CU_Com_F**: `Comercializac` Comercialización Gas Fija 

### Componentes Residuos del periodo (ton) Aseo
69.  **As_Ton_NoAp**: `No Aprov - Ordinarios` Toneladas No Aprovechables (Aseo).
70.  **As_Ton_Barr**: `Barrido y limpieza` Toneladas Barrido y Limpieza (Aseo).
71. **As_Ton_Limp**: `Limpieza urbana` Toneladas Limpieza Urbana (Aseo).
72. **As_Ton_Rech**: `Rechazados` Toneladas Rechazados (Aseo).
...
73. **Alertas**: Auditoría de cuadre: **NORMAL** o **AUDITORIA**.
74. **Info_Reg_Json**: En formato JSON. Detalles adicionales no considerados y que no tengan una columna específica asignada.
...
La salida debe ser exclusivamente una tabla Markdown. No incluyas introducciones ni conclusiones. El ancho de la tabla excederá la pantalla; esto es correcto y deseado.
